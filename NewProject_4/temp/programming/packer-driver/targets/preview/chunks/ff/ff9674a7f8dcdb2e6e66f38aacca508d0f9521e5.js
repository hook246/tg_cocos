      ...
      _export("earnView", earnView = (_dec = ccclass("earnView"), _dec2 = property(Prefab), _dec3 = property(Prefab), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property({
        ...
        initSocialTaskNode(info) {
          ...
          info.taskResponseList.forEach((task, index) => {
            ...
            child.on(Button.EventType.CLICK, /*#__PURE__*/_asyncToGenerator(function* () {
              if (_this3.taskInfo.data.data.taskModelResponses[0].taskResponseList[index].status === 3 || _this3.taskInfo.data.data.taskModelResponses[0].taskResponseList[index].status === 0) {
                return;
              }

              if (!(twitterTaskId.indexOf(_this3.taskInfo.data.data.taskModelResponses[0].taskResponseList[index].taskId) && (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                error: Error()
              ...
              if (_this3.taskInfo.data.data.taskModelResponses[0].taskResponseList[index].status === 1) {
        ...
        initDailyTaskNode(info) {
          ...
          info.taskResponseList.forEach((task, index) => {
            ...
            child.on(Button.EventType.CLICK, /*#__PURE__*/_asyncToGenerator(function* () {
              if (_this4.taskInfo.data.data.taskModelResponses[0].taskResponseList[index].status === 3 || _this4.taskInfo.data.data.taskModelResponses[0].taskResponseList[index].status === 0) {
                return;
              }

              if (_this4.taskInfo.data.data.taskModelResponses[0].taskResponseList[index].status === 1) {
                //跳转大厅
        ...
        updateSocialTaskUiByState(info, node) {
          node.getChildByName('count').getComponent(Label).string = info.taskReward;
          node.getChildByName('desc').getComponent(Label).string = info.taskDesc; // if(info.status === 1){
...
//# sourceMappingURL=ff9674a7f8dcdb2e6e66f38aacca508d0f9521e5.js.map
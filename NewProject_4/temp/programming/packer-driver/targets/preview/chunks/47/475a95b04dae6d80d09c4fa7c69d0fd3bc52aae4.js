      ...
      _export("earnView", earnView = (_dec = ccclass("earnView"), _dec2 = property(Prefab), _dec3 = property(Prefab), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property({
          ...
          return _asyncToGenerator(function* () {
            ...
            yield window.axios(config).then(taskList => {
              if (taskList.code === "200") {
                _this.changeDailyOrSocialTaskState(2);
              }
            }).catch(error => {
              console.log(error);
            });
        ...
        checkIn(index) {
          ...
          return _asyncToGenerator(function* () {
            ...
            yield window.axios(config).then(taskList => {
              if (taskList.code === "200") {
                _this2.changeCheckTaskState(index);
              }
            }).catch(error => {
              console.log(error);
            });
        ...
        claimReward(id) {
          ...
          return _asyncToGenerator(function* () {
            ...
            yield window.axios(config).then(taskList => {
              if (taskList.code === "200") {
                _this3.changeDailyOrSocialTaskState(3);
              }
            }).catch(error => {
              console.log(error);
            });
        ...
        initTaskNode(info) {
          ...
          info.data.checkInList.forEach((task, index) => {
            ...
            this.check_btns[index].on(Button.EventType.CLICK, /*#__PURE__*/_asyncToGenerator(function* () {
              yield _this4.clickCheckTaskNode(index);
            }), this);
          });
        ...
        initSocialTaskNode(info) {
          ...
          info.taskList.forEach((task, index) => {
            ...
            child.on(Button.EventType.CLICK, /*#__PURE__*/_asyncToGenerator(function* () {
              if (_this5.taskInfo.data.taskModelResponses[0].taskList[index].status === 1) {
                window.Telegram.WebApp.openLink(task.taskLink, {
                  tryInstantView: true
                });
                yield _this5.completeTask(task.taskId);

                _this5.changeDailyOrSocialTaskState(2, 2);
              }

              if (_this5.taskInfo.data.taskModelResponses[0].taskList[index].status === 2) {
                yield _this5.claimReward(task.taskId);

                _this5.changeDailyOrSocialTaskState(2, 3);
              }
            }), this);
        ...
        initDailyTaskNode(info) {
          ...
          info.taskList.forEach((task, index) => {
            var child = instantiate(this.daily_prefab);
            child.on(Button.EventType.CLICK, /*#__PURE__*/_asyncToGenerator(function* () {
              if (_this6.taskInfo.data.taskModelResponses[0].taskList[index].status === 1) {
                //跳转大厅
                _this6.discoverPage.setPosition(0, 0);

                yield _this6.completeTask(task.taskId);

                _this6.changeDailyOrSocialTaskState(3, 2);
              }

              if (_this6.taskInfo.data.taskModelResponses[0].taskList[index].status === 2) {
                yield _this6.claimReward(task.taskId);

                _this6.changeDailyOrSocialTaskState(3, 3);
              }
            }), this);
...
//# sourceMappingURL=475a95b04dae6d80d09c4fa7c69d0fd3bc52aae4.js.map
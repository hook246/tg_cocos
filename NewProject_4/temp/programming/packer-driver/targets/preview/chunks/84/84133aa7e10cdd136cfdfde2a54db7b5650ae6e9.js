    ...
    execute: function () {
      ...
      _export("gamesScrollControl", gamesScrollControl = (_dec = ccclass('gamesScrollControl'), _dec2 = property(Node), _dec3 = property({
        ...
        onLoad() {
          ...
          this.isTestX = false;
          this._touchId = null;
          this.currentIndex = 0; //this.scrollTo(0, false);
        }
        ...
        */
        ...
        update(deltaTime) {
          ...
          var lx = this.childs[this.moveAim].getPosition().x;
          ...
          var cx = this.childs[this.moveAim].getPosition().x;

          if (lx * cx < 0 && Math.abs(cx) < this.deltaX) {
            this.isTestX = false;

            for (var _i = 0; _i < this.childs.length; _i++) {
              if (Math.abs(this.childs[_i].getPosition().x) <= Math.abs(stepx)) {
...
//# sourceMappingURL=84133aa7e10cdd136cfdfde2a54db7b5650ae6e9.js.map
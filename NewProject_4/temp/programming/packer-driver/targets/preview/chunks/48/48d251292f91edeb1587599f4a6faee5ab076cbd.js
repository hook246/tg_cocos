  ...
  return {
    ...
    execute: function () {
      ...
      _export("LogicCtl", LogicCtl = (_dec = ccclass('LogicCtl'), _dec2 = property({
        ...
        next_box() {
          //获取当前最后一个箱子坐标
          var last_position = this.box_arr[this.box_arr.length - 1].node.getPosition(); //计算获取向量

          var nor_v3 = last_position.clone().subtract(this.box_arr[this.box_arr.length - 2].node.getPosition()).normalize(); //随机,x,y

          var r_x_y = Math.random() * 150 + 300; //随机箱子方向
          ...
          var new_position = new Vec3(last_position.x + nor_v3.x * r_x_y, last_position.y + nor_v3.y * r_x_y); //创建箱子,并且执行箱子初始动画
        ...
        move() {
          //获取倒数一箱子坐标
          var first = this.box_arr[this.box_arr.length - 1].node.getPosition(); //获取倒数二箱子坐标

          var second = this.box_arr[this.box_arr.length - 2].node.getPosition(); //向量计算

          var mid_position = first.subtract(second); //计算出两个箱子的中心点

          mid_position.x = mid_position.x / 2 + second.x;
          mid_position.y = mid_position.y / 2 + second.y; //获取对应的世界坐标

          var world_position = this.node.getComponent(UITransform).convertToWorldSpaceAR(mid_position); //根据世界坐标转换成canvas下的坐标

          var canvas_position = this.node.getParent().getComponent(UITransform).convertToNodeSpaceAR(world_position); //根据目标点坐标,计算出来差值

          var pos = this.light_house.getPosition().subtract(canvas_position); //执行移动动画
        ...
        create_box(pos) {
          //实例化箱子
          var ctl = instantiate(this.box_pre).getComponent(_crd && BoxCtl === void 0 ? (_reportPossibleCrUseOfBoxCtl({
            error: Error()
          }), BoxCtl) : BoxCtl); //随机箱子
        ...
        run_game(game_state) {
          ...
          var box_1 = this.create_box(new Vec3(-133, -207));
        ...
        auto_jump() {
          ...
          var pos_hero = this.hero_ctl.node.getPosition(); //获取目标箱子坐标

          var pos_box = this.box_arr[this.box_arr.length - 1].get_jump_position().subtract(pos_hero).normalize(); //执行跳跃
...
//# sourceMappingURL=48d251292f91edeb1587599f4a6faee5ab076cbd.js.map
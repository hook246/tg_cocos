    ...
    execute: function () {
      ...
      _export("BoxCtl", BoxCtl = (_dec = ccclass('BoxCtl'), _dec(_class = class BoxCtl extends Component {
        ...
        random_box(parent, pos) {
          var index = Math.random() * (this.node.children.length - 1); //27

          index = Math.round(index);
          this.cur_box = this.node.children[index];
          this.cur_box.active = true;

          for (var ele of this.node.children) {
            if (ele == this.cur_box) {
              continue;
            }
        ...
        get_jump_position() {
          var world_pos = this.cur_box.getChildByName("jump_point").getWorldPosition();
        ...
        touch_start(delta_time) {
          var y = this.node.getScale().y - delta_time / 6;
        ...
        touch_end() {
          var scale_y = 1 - this.node.getScale().y;
          var t1 = tween(this.node).to(scale_y / 5, {
            scale: new Vec3(this.node.getScale().x, 1 + scale_y / 2)
          });
          var t2 = tween(this.node).to(scale_y / 8, {
            scale: new Vec3(this.node.getScale().x, 1 - scale_y / 5)
          });
          var t3 = tween(this.node).to(scale_y / 5, {
...
//# sourceMappingURL=8d87f896effece94f0c95cd83b5d6e4b757f2d27.js.map
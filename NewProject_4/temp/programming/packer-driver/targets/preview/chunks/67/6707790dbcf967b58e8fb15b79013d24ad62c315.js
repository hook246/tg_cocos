    ...
    execute: function () {
      ...
      _export("HeroCtl", HeroCtl = (_dec = ccclass('HeroCtl'), _dec2 = property({
        ...
        touch_start(delta_time, box_height) {
          var y = this.node.getScale().y - delta_time / 6; //英雄的压缩效果
          ...
          var x = this.node.getScale().x + delta_time / 10;
        ...
        jump_by_box(target_box, touch_time, callback) {
          ...
          var jump_postion = target_box.get_jump_position(); //hero postion

          var hero_postion = this.node.getPosition(); //归一化

          var nor_v3 = jump_postion.subtract(hero_postion).normalize();
          touch_time *= 8; //落点

          var target_postion = new Vec3(hero_postion.x + nor_v3.x * touch_time, hero_postion.y + nor_v3.y * touch_time);
        ...
        jump_by_postion(target_postion, angle_x, callback) {
          //执行跳跃
          var jumpping = this.node.jump_to(target_postion, 200, 0.4); // 旋转

          var jump_start = tween(this.node).call(() => {
            //跳跃前关闭碰撞
            this.close_collider();
          ...
          var jump_end = tween(this.node).call(() => {
            //到达跳跃点后,打开碰撞
        ...
        show_score(score) {
          ...
          var sprite = this.light.getComponent(Sprite);
          sprite.color = new Color(sprite.color.r, sprite.color.g, sprite.color.b, 255);
          var base_color = sprite.color.clone();
          var temp_color = new Color();
          tween(sprite).to(0.4, {
            color: new Color(base_color.r, base_color.g, base_color.b, 0)
          }, {
            onUpdate: (target, ratio) => {
              var alpha = base_color.a + 50 - base_color.a * ratio;
        ...
        miss(target, original) {
          var target_pos = target.getComponent(_crd && BoxCtl === void 0 ? (_reportPossibleCrUseOfBoxCtl({
            error: Error()
        ...
        jump_start_effect() {
          ...
          var ball = instantiate(this.pre_ball);
          {
            //颜色RGB随机,透明度随机
          ...
          var r_x = Math.random() * (200 - -200) + -200;
          var r_y = Math.random() * (250 - -50) + -50;
        ...
        jump_end_effect() {
          for (var i = 0; i < 30; i++) {
            ...
            var r_x = Math.random() * (150 - -110) + -110;
            var r_y = Math.random() * (150 - -50) + -50;
            ball.getComponent(UITransform).setContentSize(10, 10);
            ball.setParent(this.node.getChildByName("ball_group"));
...
//# sourceMappingURL=6707790dbcf967b58e8fb15b79013d24ad62c315.js.map
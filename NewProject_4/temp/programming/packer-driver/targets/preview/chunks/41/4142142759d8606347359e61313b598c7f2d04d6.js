    ...
    execute: function () {
      ...
      _export("drawModel", drawModel = (_dec = ccclass("drawModel"), _dec(_class = class drawModel extends (_crd && basePageModel === void 0 ? (_reportPossibleCrUseOfbasePageModel({
        ...
        setTimerCount() {
          ...
          return _asyncToGenerator(function* () {
            for (var index = Math.round(_this2.drawPage.drawLuckyInfo.drawTime / 1000); index >= 0; index--) {
              ...
              var current_time = _this2.formatTimestamp(index);

              _this2.drawPage.timer.string = current_time;

              if (index <= 0) {
                var drawLuckyInfo = yield _this2.getDrawLuckInfo();

                _this2.drawPage.setDrawLuckInfo(drawLuckyInfo.data.data);

                _this2.drawPage.showDrawLuckRewardInfo(drawLuckyInfo.data.data);

                _this2.drawPage.setDrawLuckInfo(drawLuckyInfo.data.data);

                _this2.timerLessTag = false;

                if (Number(_this2.drawPage.tickets.string) < _this2.drawPage.cost_tickets) {
                  _this2.drawPage.setJoinBtnUseAble(false);

                  _this2.drawPage.setJoinTips(Number(_this2.drawPage.tickets.string) - _this2.drawPage.cost_tickets);
                } else {
                  _this2.drawPage.setJoinBtnUseAble(true);
                }

                yield _this2.setTimerCount();
              } else if (index <= 5 && index > 0) {
                _this2.drawPage.setJoinBtnUseAble(false);

                _this2.timerLessTag = true;
              }
            }
          })();
        }
        ...
        formatTimestamp(timestamp) {
          ...
          var hours = Math.floor(timestamp % (3600 * 24) / 3600);
        ...
        initData() {
          ...
          return _asyncToGenerator(function* () {
            ...
            var config = yield _this4.searchConfig();
            _this4.drawPage.cost_tickets = config.data.data.ticketCost;
            var ticketsInfo = yield _this4.getUserTicketsUsdtInfo();

            _this4.drawPage.setJoinTips(ticketsInfo.data.data.tiket - config.data.data.ticketCost);

            _this4.drawPage.setCurrentTicketsUsdtCount(ticketsInfo.data);

            var drawLuckyData = yield _this4.getDrawLuckInfo();

            if (drawLuckyData.data.data.join) {
              _this4.drawPage.setJoinBtnUseAble(false);
            } else {
              _this4.drawPage.setJoinBtnUseAble(true);
            }

            _this4.drawPage.setDrawLuckInfo(drawLuckyData.data.data);

            _this4.setTimerCount();
          })();
        }
        ...
        searchConfig() {
          ...
          return _asyncToGenerator(function* () {
            ...
            _this8.drawPage.setTicketsCost(config.data.data.ticketCost);
        ...
        joinDrawLucy() {
          ...
          return _asyncToGenerator(function* () {
            try {
              ...
              if (Number(_this9.drawPage.tickets.string) < Number(_this9.drawPage.cost_tickets)) {
              ...
              _this9.drawPage.join_count.string = join.data.data.count;
              _this9.drawPage.tickets.string = join.data.data.tiket;
          ...
          return _asyncToGenerator(function* () {
            try {
              var reward = yield window.axios.post(drawLuckUrls.claim, {
                "drawId": "" + _this10.drawPage.drawLuckyInfo.openDrawId
              }, {
                headers: {
                  'Authorization': "Bearer " + (local ? local_token : (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
              ...
              _this10.drawPage.setCurrentTicketsUsdtCount(reward.data);

              _this10.drawPage.closeDrawLuckRewardInfo();

              _this10.drawPage.drawLuckyInfo.openDrawId = null;
              _this10.drawPage.drawLuckyInfo.win = false;
              window.Telegram.WebApp.showAlert("claim success!", () => {
                console.log("");
              });

              if (!_this10.timerLessTag) {
                _this10.drawPage.setJoinBtnUseAble(true);
              }
            } catch (error) {
...
//# sourceMappingURL=4142142759d8606347359e61313b598c7f2d04d6.js.map